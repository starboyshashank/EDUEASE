import React from 'react';


const HowItWorks = () => {
  
  return (
    <>
    <section>
      <div className="work-container container">
        <h1>THE NEED TO-<span className="join"> COME TOGETHER TO</span> <br /> <span className="care"> SUPPORT OUR GIRLS</span></h1>
        <p className="initiative">
          It is estimated that there are 15 million females in India who are in need of Support and Education.<br/><br/>
          It is a large number. But, at Make A Difference, we believe that the way to solve this problem is to build a community around these women. A community of those who 
          care and want to contribute in whichever way then can to help build long-term solutions to this problem.<br/><br/>
          Imagine what a collective of 1 million doers and believers will be able to do. We can. And we can't wait to get started!
        </p>

    <div className='row'>
    <div className="col-12 col-lg-4 text-center work-container-subdiv">
    <i class="bi bi-currency-dollar"></i>
    <h2 className="sub-heading">Donate Money</h2>
    <p className="main-hero-para w-100">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?
    </p>
     </div>
     <div className="col-12 col-lg-4 text-center work-container-subdiv">
     <i class="bi bi-book-half"></i>
    <h2 className="sub-heading">Donate Books</h2>
    <p className="main-hero-para w-100">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?
    </p>
     </div>
     <div className="col-12 col-lg-4 text-center work-container-subdiv">
     <i class="bi bi-globe"></i>
    <h2 className="sub-heading">Volunteer</h2>
    <p className="main-hero-para w-100">Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?Lorem ipsum, dolor sit amet consectetur adipisicing elit. Etquia quis?
    </p>
     </div>
     </div>
      </div>

<div className="puducherry">
      <table className="fuddu" data-aos="zoom-in">
   <thead>
     <tr>
       <th>Category</th>
       <th>Country/State</th>
       <th>Literacy Rate(2001 year)</th>
       <th>Literacy Rate(2011 year)</th>
     </tr>
   </thead>
   <tbody>
      <tr>
         <td>State/ Union Territory</td>
         <td>Country/State</td>
         <td><img className="graph2" src="images/graph1.png" alt=""/></td>
         <td><img className="graph1" src="images/graph2.webp" alt=""/></td>
       </tr>
       <tr>
           <td>State</td>
           <td>Andhra Pradesh</td>
           <td>60.5</td>
           <td>67</td>
           <td></td>
         </tr>
         <tr>
           <td>State</td>
           <td>Arunachal Pradesh</td>
           <td>54.3</td>
           <td>65.4</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Assam</td>
           <td>63.3</td>
           <td>72.2</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Bihar</td>
           <td>47</td>
           <td>61.8</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Chhattisgarh</td>
           <td>64.7</td>
           <td>70.3</td>
         </tr>
        
         <tr>
           <td>State</td>
           <td>Gujarat</td>
           <td>69.1</td>
           <td>78</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Haryana</td>
           <td>67.9</td>
           <td>75.6</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Himachal Pradesh</td>
           <td>76.5</td>
           <td>82.8</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Jammu and Kashmir</td>
           <td>55.5</td>
           <td>67.2</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Jharkhand</td>
           <td>53.6</td>
           <td>66.4</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Karnataka</td>
           <td>66.6</td>
           <td>75.4</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Kerala</td>
           <td>90.9</td>
           <td>94</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Madhya Pradesh</td>
           <td>63.7</td>
           <td>69.3</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Maharashtra</td>
           <td>76.9</td>
           <td>82.3</td>
         </tr>
       
      <tr>
           <td>State</td>
           <td>Odisha</td>
           <td>63.1</td>
           <td>72.9</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Punjab</td>
           <td>69.7</td>
           <td>75.8</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Rajasthan</td>
           <td>60.4</td>
           <td>66.1</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Sikkim</td>
           <td>68.8</td>
           <td>81.4</td>
         </tr>
         <tr>
           <td>State</td>
           <td>Tamil Nadu</td>
           <td>73.5</td>
           <td>80.1</td>
         </tr>
         
         <tr>
           <td>State</td>
           <td>Uttar Pradesh</td>
           <td>56.3</td>
           <td>67.7</td>
         </tr>
     
         <tr>
           <td>State</td>
           <td>West Bengal</td>
           <td>68.6</td>
           <td>76.3</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>A & N Islands</td>
           <td>81.3</td>
           <td>86.6</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>Chandigarh</td>
           <td>81.9</td>
           <td>86</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>D & N Haveli</td>
           <td>57.6</td>
           <td>76.2</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>Daman & Diu</td>
           <td>78.2</td>
           <td>87.1</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>Lakshadweep</td>
           <td>86.7</td>
           <td>91.8</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>NCT of Delhi</td>
           <td>81.7</td>
           <td>86.2</td>
         </tr>
         <tr>
           <td>Union Territory</td>
           <td>Puducherry</td>
           <td>81.2</td>
           <td>85.8</td>
         </tr>
   </tbody>
 </table>
</div>
    </section>


    
    </>
  );
};

export default HowItWorks;